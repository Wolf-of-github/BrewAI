#!/usr/bin/env bash
# Build, push, and deploy the generative-ai-service to Cloud Run.
# Run from the infra/ directory: ./deploy-generative-ai.sh [image-tag]
set -euo pipefail
...
gcloud run services update "${SERVICE}" \
  ...
  --region="${REGION}" \
  --project="${PROJECT_ID}" \
  --update-env-vars="SERVICE_VERSION=${TAG}" \
  --quiet
...
echo "==> Done. Generative AI service deployed with tag=${TAG}"
