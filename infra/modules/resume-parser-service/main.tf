...
resource "google_artifact_registry_repository" "repo" {
  repository_id = "resume-parser-service"
  location      = var.region
  format        = "DOCKER"
...
resource "google_service_account" "sa" {
  ...
  display_name = "Resume Parser Service SA"
  project      = var.project_id
}
...
resource "google_cloud_run_v2_service" "app" {
  ...
  template {
    ...
    containers {
      image = "${var.region}-docker.pkg.dev/${var.project_id}/${google_artifact_registry_repository.repo.repository_id}/resume-parser-service:${var.image_tag}"
...
resource "google_cloud_tasks_queue" "parse" {
  name     = "resume-parser-service-parse"
  location = var.region
  project  = var.project_id
